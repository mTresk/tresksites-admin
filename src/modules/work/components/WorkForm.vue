...
const { mutate: mutateUpdate } = useMutation({
	mutationFn: async () => {
		errors.value = await update(`works/${route.params.slug}`, work.value)
	},
	onSuccess: async () => {
...
watch(
	() => work.value.name,
	() => !props.workProps?.slug ? work.value.slug = slug(work.value.name) : work.value.slug = props.workProps?.slug,
)
			...
			<template #content>
				<VForm footer :is-loading="isLoading" @form-submit="props.workProps?.id ? handleUpdate() : handleCreate()">
					<template #body>
						...
						<VFormField wide>
							<label class="form__label" for="list">Список</label>
							<Editor
								id="list"
								v-model="work.list"
								editor-style="font-size: 15px"
									...
									<div class="card__body p-card">
										<div class="card__block">
											...
											<Editor
												...
												editor-style="min-height: auto; font-size: 15px"
											>
												<template #toolbar>
													<span class="ql-formats">
														<select class="ql-header">
					...
					<template #body>
						<VFormField wide>
							<span class="p-float-label">
								<InputText
									id="work.seo.title"
									v-model="work.seo.title"
									type="text"
									aria-describedby="text-error"
								/>
								<label for="work.seo.title">SEO заголовок</label>
							</span>
						</VFormField>
						<VFormField wide>
							<span class="p-float-label">
								<Textarea
									id="work.seo.description"
									v-model="work.seo.description"
									rows="5"
									type="text"
									aria-describedby="text-error"
								/>
								<label for="work.seo.description">SEO описание</label>
							</span>
						</VFormField>
					</template>
				</VForm>
